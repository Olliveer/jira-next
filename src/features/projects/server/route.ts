...
const app = new Hono()
  ...
  .get('/:projectId', sessionMiddleware, zValidator('param', z.object({ projectId: z.string() })), async c => {
    const user = c.get('user');
    const databases = c.get('databases');
  ...
  .get(
    ...
    async c => {
      ...
      const thisMonthAssigneesTasks = await databases.listDocuments(DATABASE_ID, TASKS_ID, [
        Query.equal('projectId', projectId),
      ...
      const lastMonthAssigneesTasks = await databases.listDocuments(DATABASE_ID, TASKS_ID, [
        Query.equal('projectId', projectId),
        Query.equal('assigneeId', member.$id),
        Query.greaterThanEqual('$createdAt', lastMonthStart.toISOString()),
        Query.lessThanEqual('$createdAt', lastMonthEnd.toISOString()),
      ...
      const assigneeCount = thisMonthAssigneesTasks.total;
      const assigneeDiff = assigneeCount - lastMonthAssigneesTasks.total;

      const thisMonthInconpleteTasks = await databases.listDocuments(DATABASE_ID, TASKS_ID, [
        Query.equal('projectId', projectId),
      ...
      const lastMonthInconpleteTasks = await databases.listDocuments(DATABASE_ID, TASKS_ID, [
        Query.equal('projectId', projectId),
        Query.notEqual('status', TaskStatus.DONE),
        Query.greaterThanEqual('$createdAt', lastMonthStart.toISOString()),
        Query.lessThanEqual('$createdAt', lastMonthEnd.toISOString()),
      ...
      const inconpleteTaskCount = thisMonthInconpleteTasks.total;
      const inconpleteTaskDiff = inconpleteTaskCount - lastMonthInconpleteTasks.total;
